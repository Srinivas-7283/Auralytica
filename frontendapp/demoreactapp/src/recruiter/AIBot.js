...
import { Bot, User, Paperclip, Send, Download, Sparkles, MessageSquare, ShieldCheck, XCircle, Briefcase } from "lucide-react";
...
export default function AIBot() {
  ...
  const recruiter = JSON.parse(localStorage.getItem("recruiter")); // Using recruiter from local storage
    ...
    const loadHistory = async () => {
      try {
        const res = await axios.get(
          `${config.url}/chat-history?userEmail=${recruiter.email}&role=recruiter`
        );
  ...
  const handleResumeUpload = (e) => {
    ...
    if (file) {
      ...
      setChat(prev => [...prev, {
        sender: "system",
        text: `📎 Candidate profile attached: ${file.name}`
      }]);
    }
  };
  ...
  const sendMessage = async () => {
    ...
    try {
      if (resume) {
        ...
        formData.append("userEmail", recruiter.email);
      ...
      setChat(prev => [...prev, {
        sender: "bot",
        text: "Neural processing interrupted. Please initiate the request again."
      }]);
    }
  ...
  const downloadLastResponse = async () => {
    ...
    try {
      ...
      link.href = url;
      link.download = `Hiring_Insights_${new Date().getTime()}.pdf`;
      link.click();
    } catch (err) {
  ...
  const downloadAllQuestions = async () => {
    ...
    try {
      ...
      const link = document.createElement("a");
      link.href = url;
      link.download = "Recruitment_Assistant_Transcript.pdf";
      link.click();
    } catch (err) {
      alert("Failed to generate PDF");
    }
    ...
    <div style={styles.container}>
      <div style={styles.header} className="animate-fade-up">
        <h1 style={styles.title} className="font-heading">
          <Briefcase size={32} style={{ color: 'var(--primary)', marginRight: '15px' }} />
          <span className="gradient-text">AI Hiring Assistant</span>
        </h1>
        <p style={styles.subtitle}>Strategize your hiring decisions with intelligent interview guidance</p>
      </div>
        ...
        <div style={styles.contextBar}>
          {resumeUploaded ? (
            <div style={styles.contextBadge}>
              ...
              <span style={{ fontSize: '0.8rem', fontWeight: '600' }}>Candidate Analysis Enabled: {resume.name}</span>
            ...
            <label style={styles.uploadLabel}>
              <Paperclip size={14} />
              Attach Candidate Resume for Deep Analysis
        ...
        <div ref={chatBoxRef} style={styles.chatBox}>
          {chat.length === 0 && (
            <div style={styles.emptyState}>
              ...
              <h3 className="font-heading" style={{ fontSize: '1.2rem', marginBottom: '10px' }}>Strategy Engine Initialized</h3>
              <p style={{ fontSize: '0.9rem', color: 'var(--text-muted)' }}>
                Generate role-specific interview questions, analyze candidate fit, or get strategic hiring advice instantly.
              </p>
            </div>
          )}
        ...
        <div style={styles.inputArea}>
          <div style={styles.inputContainer}>
            <input
              ...
              placeholder={resumeUploaded ? "Inquire about the candidate's skills..." : "Command your assistant..."}
              style={styles.input}
              disabled={loading}
            />
            <button
